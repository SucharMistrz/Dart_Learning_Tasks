...
void main(List<String> arguments) async {
  ...
  for (int row = 0; row < fileContent.length; row++) {
    for (int column = 0; column < fileContent[row].length; column++) {
      //Find the first letter of the keyword
      if (fileContent[row][column] != keyword[0]) {
        continue;
      }

      //Determine which directions have to be checked
      bool goUp = false;
      bool goDown = false;
      bool goLeft = false;
      bool goRight = false;
  ...
  //This code will work with any random input
  //but if the keyword is a palindrome, then every instance will be found twice (as normal AND backwards)!
